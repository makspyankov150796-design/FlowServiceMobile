...
//------------------формируем имя файла ----------------------
  ...
  do
  {
    ...
    */
    ...
    rate = static_cast<double>(verifyData.totalPulsesCnt[0]) / verifyData.MeasureData.time;
 ...
 void __fastcall TPhotoFixForm::UpdateState(void)
  ...
  //Запуск измерения
	case 2:
	{
	  ...
	  EtalonMeter->VolumeValue->SetValue((double)0);
		...
		if (state == 2)
	{
	  ...
	  DurationTime = (double) MilliSecondsBetween(BeginTime,EndTime)/1000;   //MilliSecondsBetween(EndTime, BeginTime)/1000;
          ...
          bool &Handled)
{
	 if (EventInfo.Flags.Contains(TInteractiveGestureFlag::gfBegin))
		{
			...
			InitialDistance = (double) EventInfo.Distance;
	...
	switch (EventInfo.GestureID)
	{
...

		{
			...
			double scaleFactor = (double) (EventInfo.Distance) / (double)(InitialDistance);
			int newWidth = InitialWidth * scaleFactor;
			int newHeight = InitialHeight * scaleFactor;

			 double scale =  (double) newWidth/InitialWidth;
