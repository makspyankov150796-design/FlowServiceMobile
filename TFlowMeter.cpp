...
void  TFlowMeter::SetMonitorValues(void)
{
	 ...
	 FlowValue -> SetValue((double)GetFlow());
	 VolumeValue -> SetValue((double)GetFlowVolume());
	 ImpValue -> SetValue((double)GetTotalImp());
...
{
	 FlowValue -> SetValue((double)GetFlow());
	 VolumeValue -> SetValue((double)GetVolume());
	 ImpValue -> SetValue((double)GetTotalImp());
	 VolumeErrorValue -> SetValue((double)GetVolumeError());
...
void  TFlowMeter::SetFinalValues(void)
{
	 ...
	 FlowValue -> SetValue((double)GetFinalFlow());
	 VolumeValue -> SetValue((double)GetFinalVolume());
	 ImpValue -> SetValue((double)GetFinalImp());

	 VolumeErrorValue -> SetValue((double)GetVolumeError());
}

double TFlowMeter::GetFlowVolume(void)
{

	double Vol, flow, CalibrVol;
	double calibr_flow, rate;
...
double TFlowMeter::GetWidthFlowVolume(void)
{
...
double TFlowMeter::GetTotalImp(void)
{

    return HSCDevice->GetTotalImp(Channel);
}

double TFlowMeter::GetVolume(void)
{   double Vol, flow, CalibrVol;
...
double TFlowMeter::GetFinalRawVolume(void)
{   double Vol;
...
double TFlowMeter::GetFinalVolume(void)
{
    double Vol, flow, CalibrVol;
...
double TFlowMeter::GetRawWidthFlow(void)
{
	double flow, calibr_flow, rate;
...
double TFlowMeter::GetWidthFlow(void)
{
	double flow, calibr_flow, rate;
...
double TFlowMeter::GetFlow(void)
{
	double flow, calibr_flow, rate;
...
double TFlowMeter::GetFinalRawFlow(void)
{
	double flow;
...
double TFlowMeter::GetFinalImp(void)
{

	return (double)HSCDevice->GetFinalImp(Channel);
}

double TFlowMeter::GetFinalFlow(void)
{
	double flow, calibr_flow, rate;
...
bool TFlowMeter::GetPointUseFlow(double Q, tPoint &point)
{
...
uint8_t TFlowMeter::AddPointData (UnicodeString Name, double Qrate, double Q, double Volume, double vTime, double Error,double RagePlus, double RageMinus )
{
	tPoint Pnt;
	uint8_t error;
...
int8_t TFlowMeter::AddCalibrData(double vEtlVolume, double vTime, double vImp, double vCoef)
 {
    tCalibrPoint Pnt;
...
int8_t TFlowMeter::UpdateCoefs()
 ...
 double Qtest1, Qetl1, Qtest2, Qetl2;
 double k,b;
...
double  TFlowMeter::Rate(double Q)
{
	double temp,k,b;
    double Qetl=0;
    double rate;
				...
				for (int i = 0; i < FlowMeters[j]->DataPoints.size(); i++) {
				   ...
				   /*	sampleNode2->SetAttribute(L"Qrate",
					...
					sampleNode2->SetAttribute(L"VolumeBefore",
						FloatToStrF((double)FlowMeters[j]->DataPoints[i].VolumeBefore,
						ffNumber, 10,5));
					sampleNode2->SetAttribute(L"VolumeAfter",
						FloatToStrF((double)FlowMeters[j]->DataPoints[i].VolumeAfter,
						ffNumber, 10, 5));
...
int TFlowMeter::TimeToEndVolumeLimit(double pointVolume,double DelayCoef, double TimeCoef)
{
	double add_volume=0, volume =0, add_time=0, flow;
...
bool TFlowMeter::IsFlowInPoint(double Q)
{
