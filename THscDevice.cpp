...
void THscDevice::SaveVerificationResults(void) {
	...
	do {
		...
		rate = static_cast<double>(verifyData.totalPulsesCnt[0])
...
double THscDevice::GetAvrImpulses(uint8_t CH) {
	uint16_t Sum = 0;
	uint8_t cnt;
	uint8_t i = 0;
...
void THscDevice::Write_Coef(double fltK)
{
	WriteReg(COMPACT_CONFIG_KP, (double)fltK);
...
double THscDevice::GetTotalImp(uint8_t CH) {
	double Imp;
	Imp = static_cast<double>(CumData.ImpTotal[CH]);
	return Imp;
}

double THscDevice::GetSecImp(uint8_t CH) {
    double SecImp;
    SecImp =  (double) SecData.ImpPerSec[CH];

	return (double)SecImp;
}

double THscDevice::GetWidth(uint8_t CH) {
	double Width;
	Width =  (double) SecData.Width[CH];

	return (double)Width;
}

double THscDevice::GetTime(void) {
	return (double)SecData.Time;
}

double THscDevice::GetFinalTime(void)  {
	return (double)FinalData.Time;
}

double THscDevice::GetFinalImp(uint8_t CH) {
	return (double)FinalData.ImpFinal[CH];
}
...
UnicodeString THscDevice::GetCharge(void)
{
	...
	f = Charge;
	d = ((f*0.026)-2.9);
	p =  (double)(d*83.3333);     //  =(((ADC*0,026)-2,9)/1,2)*100
