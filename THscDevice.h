...
typedef struct TConditions {
	double tempAir; // < Температура воздуха (°C)
	double tempWater; // < Температура рабочей жидкости (°C)
...
public:
	...
	double GetAvrImpulses(uint8_t Channel);
	uint8_t GetMaxChannel(void);
	double GetTotalImp(uint8_t Channel);
	double GetSecImp(uint8_t Channel);
	double GetTime(void);
	double GetWidth(uint8_t CH);
	double GetFinalImp(uint8_t Channel);
...
protected:
	...
	double    Widths[16][IMP_BUFFER_LEN];
