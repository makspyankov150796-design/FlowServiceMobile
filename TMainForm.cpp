...
void __fastcall TMainTabbedForm::RefreshPointsTable(void) {
	...
	String str1 = "", str2 = "";
	double Q2, Qetl, error;
	double Qrate, Flow, Volume, Time, Imp, Error, RagePlus, RageMinus;
	double f;
	bool succes = true;
	char chr = 160U;
			...
			case mrYes : {
				...
				if (!(TestMeter == NULL)) {
					...
					for (int i = 0; i < TestMeter->Points.size(); i++) {
						...
						randomNum = rand() % 100;
					   //	srand(static_cast<unsigned int>(randomNum+i+static_cast<double>(TimeStampToMSecs(DateTimeToTimeStamp(Now())))));
						randomNum = rand() % 500 + 300;
						T =  (double)randomNum / 100 ;
					   ...
					   /*	f = rand() % 350 + 50; f = f / 1000;
						...
						// srand(static_cast<unsigned int>(100+static_cast<double>(TimeStampToMSecs(DateTimeToTimeStamp(Now())))));
...
void __fastcall TMainTabbedForm::Edit6KeyDown(TObject *Sender, WORD &Key,
	...
	if (Key == 13U) {
		...
		EtalonMeter->FlowValue->SetValue((double)StrToFloat(Edit6->Text));
  ...
  //	Canvas->FillRect(Bounds, 0, 0, AllCorners, 1);
   ...
   /*	if (Column->TabOrder != 2 && Row == 1) { // Например, закрасить третий столбец и вторую строку
	...
	if (Column->Tag == 1 && Row == 1) { // Например, закрасить третий столбец и вторую строку
		...
		StringGrid1->Canvas->Fill->Color = TAlphaColorRec::Black;
		// Рисуем текст в ячейке
		StringGrid1->Canvas->FillText(Bounds, L"Qmax", false, 1.0,TFillTextFlags(), TTextAlign::Leading, TTextAlign::Leading);
		// Отменяем стандартное рисование
	   //	DefaultDrawing = false;
	 }
		...
		StringGrid1->Canvas->Fill->Color = TAlphaColorRec::Black;
		// Рисуем текст в ячейке
		StringGrid1->Canvas->FillText(Bounds, L"Qmin", false, 1.0,TFillTextFlags(), TTextAlign::Leading, TTextAlign::Leading);
		// Отменяем стандартное рисование
	   //	DefaultDrawing = false;
	 }
          ...
          const TValue &Value, const TGridDrawStates State)
{
   ...
   if (TestMeter->DataPoints.size()>Row) {
		...
		StringGrid1->Canvas->Fill->Color = TAlphaColorRec::Black;
		// Рисуем текст в ячейке
		StringGrid1->Canvas->FillText(Bounds, TestMeter->DataPoints[Row].Name, false, 1.0,TFillTextFlags(), TTextAlign::Leading, TTextAlign::Leading);
		// Отменяем стандартное рисование
			}
