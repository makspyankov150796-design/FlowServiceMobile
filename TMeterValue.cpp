...
double TMeterValue::AverageApply() {
...
double TMeterValue::GetFloatValue() {
	return value;
}
...
double TMeterValue::GetFloatValue(uint8_t Dim) {
	return GetFloatValue() * GetDimRate(Dim);
}

double TMeterValue::GetFloatValue(UnicodeString DimName) {
	double flt = GetFloatValue() * GetDimRate(DimName);
...
void TMeterValue::SetValue(double Value) {
...
UnicodeString TMeterValue::GetStringValue(uint8_t Dim) {
	...
	double flt = GetFloatValue(Dim);

	// double flt =  GetFloatValue(0);
